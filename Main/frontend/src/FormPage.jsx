...
export default function FormPage({ onBack }) {
  ...
  const [authToken, setAuthToken] = useState(localStorage.getItem("msj_token") || "");
  ...
  // PASSO 1: FAZER LOGIN/CADASTRO DE VERDADE
  if (currentStepId === "auth") {
    try {
      ...
      return; // Para aqui se deu erro no login
    }
  }
  ...
  if (phase === 1 && currentStep === phase1Steps.length - 1) {
    setIsAnimating(true);
    setTimeout(() => { setPhase(2); setCurrentStep(0); setErrors({}); setIsAnimating(false); }, 300);
    return;
  }
  if (currentStep < totalSteps - 1) {
    setIsAnimating(true);
    setTimeout(() => { setCurrentStep((prev) => prev + 1); setErrors({}); setIsAnimating(false); }, 300);
  }
};
        ...
        <div className="form-wrapper">
          ...
          <div className="form-navigation">
            <button
              ...
              onClick={handlePrev}
              disabled={isFirstStep || isProcessingIA}
            >
              <FormIcons.ArrowLeft />
              <span>Anterior</span>
            </button>
            ...
            {isLastStep ? (
              ...
              >
                <FormIcons.Rocket />
                <span>{isProcessingIA ? "Criando..." : "Criar meu site"}</span>
              </button>
            ) : (
              <button
                ...
                disabled={isProcessingIA}
              >
                <span>{isProcessingIA ? "Processando..." : "Próximo"}</span>
